...
function bossesDefeatedByWave(index: number, content: GameContent): number {
...
function gridRadiusForWave(index: number, content: GameContent): number {
  return content.config.gridRadius + bossesDefeatedByWave(index, content);
}

function buildRadiusForWave(index: number, content: GameContent): number {
  return content.config.buildRadius + bossesDefeatedByWave(index, content);
}
...
function spawnLanesForWave(index: number, content: GameContent): AxialCoord[] {
  ...
  const laneCount = Math.min(6 + bossesDefeatedByWave(index, content) * 2, 12);
...
function spawnIntervalMs(index: number, content: GameContent, modifier = 0): number {
  ...
  return Math.max(
    ...
    860 - cycle * content.config.spawnIntervalStepMs - slot * 40 + modifier
  );
}
...
function buildPatternSpawns(
  ...
  const interval = spawnIntervalMs(index, content, pattern === 'surge' ? -80 : pattern === 'staggered' ? 20 : 0);
  const cycle = cycleNumber(index, content);
  const enemies =
    pattern === 'surge'
  ...
  const minimumEnemyCount = 4 + cycle + Math.floor(slotInCycle(index, content) / 2) + (pattern === 'surge' ? 1 : 0);
  while (enemies.length < minimumEnemyCount) {
    enemies.push('raider');
  }
...
function buildPebbleSpawns(index: number, content: GameContent): WaveSpawn[] {
  const radius = gridRadiusForWave(index, content);
  const laneIndex = bossWaveOrdinal(index, content) % 6;
  return [{
    atMs: 0,
    enemyId: 'pebble',
    laneIndex,
    tile: rotateCoord({ q: 0, r: -radius }, laneIndex)
  }];
}
...
function buildEndUserHordeSpawns(index: number, content: GameContent): WaveSpawn[] {
  ...
  const spawns: WaveSpawn[] = [];
  let atMs = 0;
  const spawnCount = 18 + bossWaveOrdinal(index, content) * 4;
...
function pebblePathForLane(laneIndex: number, radius: number): AxialCoord[] {
  const extension: AxialCoord[] = [];
  for (let currentRadius = radius; currentRadius > 6; currentRadius -= 1) {
    extension.push({ q: 0, r: -currentRadius });
  }
  return [...extension, ...PEBBLE_PATH_BASE].map((coord) => rotateCoord(coord, laneIndex));
}
...
function stepPebble(state: RunState, enemy: EnemyInstance, content: GameContent): void {
  ...
  if (state.timeMs < enemy.moveReadyAtMs) return;
  const path = pebblePathForLane(enemy.spawnLaneIndex ?? 0, currentGridRadius(state, content));
...
function spawnEnemies(state: RunState, content: GameContent): void {
  ...
  for (const spawn of state.pendingSpawns) {
    ...
    const lane = spawn.tile ? { ...spawn.tile } : spawnLanes[spawn.laneIndex % spawnLanes.length];
